...
>>>>>>> branch 'master' of https://github.com/X-com/amber.git
			...
			for (Gob gob : oc) {
				try {
					...
					} else if (Config.alarmbram && (res.name.equals("gfx/terobjs/vehicle/bram")
							|| res.name.equals("gfx/terobjs/vehicle/catapult"))) {
						sgobs.add(gob.id);
	...
	public void tick(double dt) {
		...
		if (Config.playerposfile != null && MapGridSave.gul != null) {
			try {
				...
				// plcrel = pl.rc.sub((MapGridSave.gul.x + 50) * tilesz.x,
				// (MapGridSave.gul.y + 50) * tilesz.y);
