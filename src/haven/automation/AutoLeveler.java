...
public class AutoLeveler extends Window implements GobSelectCallback, ErrorSysMsgCallback {
	...
	public AutoLeveler() {
		...
		inf.add(new RichTextBox(new Coord(245, 55),
				Resource.getLocString(Resource.BUNDLE_LABEL,
		...
		public void run() {
			...
			lvl: while (!terminate) {
				try {
					 ...
					 * 
					 * IMeter.Meter stam = gameui().getmeter("stam", 0); if
					 * (stam != null && stam.a < 30) { terminate(); return; }
					 */

					Window survwnd = gui.getwnd("Land survey");
					if (survwnd == null) {
						...
						survwnd = gui.waitfForWnd("Land survey", 60 * 1000);
						if (survwnd == null)
							continue;
					}
					Field tzf = survwnd.getClass().getDeclaredField("tz"); // throw
																			// NPE
																			// if
																			// user
																			// clicks
					...
					while (!terminate) {
						...
						/*
						 * stam = gameui().getmeter("stam", 0); if (stam != null
						 * && stam.a < 30) continue lvl;
						 */
						...
						if ("You are too tired to dig.".equals(lasteerrmsg)) {
							terminate();
						}
						if ("You lack energy for hard labor.".equals(lasteerrmsg)) {
							terminate();
						}

					}
					// there was a issue here with needsoil = null when it
					// shouldn't be
					// as a result once you use the survey to put soil down the
					// script
	...
	private Gob getClosestStockpile(Set<Gob> fullstockpiles2) {
		Gob stockling = null;
		double closest = Double.MAX_VALUE;
		for (Gob s : stockpiles) {
			double dist = gui.map.player().rc.dist(s.rc);
			if (dist < closest && !fullstockpiles.contains(s)) {
				closest = dist;
				stockling = s;
			}
		}
	...
	public Gob setSurvay(OCache oc) {
		if (survey == null) {
	...
	private void storesoil() {
		...
		stockpiles:
		// for (Gob s : stockpiles) {
		while (stockpiles.size() != fullstockpiles.size()) {
			Gob s = getClosestStockpile(fullstockpiles);
			if (terminate)
				return;

			if (s == null || fullstockpiles.contains(s))
				continue;
			...
			for (Widget w = spwnd.lchild; w != null; w = w.prev) {
				if (w instanceof ISBox) {
					...
					if (freespace > 0) {
						// we use boolean instead incase the ISBox didn't load
						// yet
						nospace = false;
						break;
					} else {
	...
	private boolean findObject(Gob s) {
		Glob glob = gui.map.glob;
		synchronized (glob.oc) {
			if (glob.oc.getgob(s.id) == null)
				return false;
	...
	private Gob getClosestStockpile() {
		Gob stockling = null;
		double closest = Double.MAX_VALUE;
		for (Gob s : stockpiles) {
			double dist = gui.map.player().rc.dist(s.rc);
			if (dist < closest && findObject(s)) {
				closest = dist;
				stockling = s;
			}
		}
	...
	private void getsoil() {
		GameUI gui = gameui();
		// for (Gob s : stockpiles) {
		while (stockpiles.size() > 0) {
			Gob s = getClosestStockpile();
			if (s == null || terminate)
				return;
